# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/00_primitives.ipynb.

# %% auto 0
__all__ = ['plot_graph', 'plot_graph_as_quiver', 'graph2networkx_with_pos', 'is_in_edges_', 'VoronoiRegions2Edges', 'Vertex',
           'Graph', 'Monolayer', 'mask2vrn_region', 'unit_hex_seeds', 'unit_hexagons', 'generate_lattice']

# %% ../nbs/00_primitives.ipynb 3
import torch
import numpy as np, matplotlib.pyplot as plt, networkx as nx
from scipy.spatial import Voronoi
# for plotting Voronoi use scipy.spatial.voronoi_plot_2d

# %% ../nbs/00_primitives.ipynb 5
...
# %% ../nbs/00_primitives.ipynb 40
def generate_lattice(Ny, Nx, p_noise=0.25, rng_seed=None):
    ...
    probability `p_noise` (default: 0.25). A `p_noise` zero is equvalent to using `unit_hexagons()`.
    ...
    '''
    Xseed = unit_hex_seeds(Nx+2,Ny+2)
    Xseed_min = Xseed.min(axis=0)
    Xseed_max = Xseed.max(axis=0)
    # inner seeds (to keep and to add noise)
    Xmask_inner = np.logical_and(
        np.logical_and( Xseed[:,0] > Xseed_min[0], Xseed[:,1] > Xseed_min[1]),
        np.logical_and( Xseed[:,0] < Xseed_max[0], Xseed[:,1] < Xseed_max[1])
    )

    if p_noise:
        rng = np.random.default_rng(rng_seed)
        # mask with prob-y p_noise for inner seeds
        Xnoise_mask = rng.uniform(size=(Xseed.shape[0],)) < p_noise
        Xnoise_mask[np.logical_not(Xmask_inner)] = False
        # add noise to inner seeds
        Xseed[ Xnoise_mask, :] = rng.uniform( size=(Xnoise_mask.sum(), 2)) * ( Xseed[ Xmask_inner, :].max(axis=0) - Xseed[ Xmask_inner, :].min(axis=0))
    
    # get Voronoi diagram
    vrn = Voronoi(Xseed)

    vrn_mask = np.logical_and(
        np.logical_and( vrn.vertices[:,0] > Xseed_min[0], vrn.vertices[:,1] > Xseed_min[1]),
        np.logical_and( vrn.vertices[:,0] < Xseed_max[0], vrn.vertices[:,1] < Xseed_max[1])
    )
    
    v_x, regions = mask2vrn_region(vrn.vertices, vrn.regions, vrn_mask)
    vrn_mask = np.ones(v_x.shape[0],dtype=bool)
    vrn_mask[[k for k in range(v_x.shape[0]) if k not in np.unique(np.concatenate(regions))]] = False
    v_x, regions = mask2vrn_region(v_x, regions, vrn_mask)
